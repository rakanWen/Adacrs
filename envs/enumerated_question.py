...
import random
import torch
from adacrs.utils.utils_load_save import load_embed, set_random_seed, DATA_DIR
...
class EnumeratedRecommendEnv(object):
    ...
    def __init__(self,
        ...
        ]  # user accepted large_feature which asked by agent
        self.user_rej_feature = [
        ]  # user rejected large_feature which asked by agent
        self.acc_small_fea = []
        self.rej_small_fea = []
        self.rej_item = []
        ...
        for fea in self.kg.G['large_feature']:
            self.valid_small_feature.extend(
                self.kg.G['large_feature'][fea]['link_to_feature'])
        self.invalid_small_feature = set(self.kg.G['feature'].keys()) - set(
            self.valid_small_feature)
        ...
        self.cur_node_set = [
        ]  # maybe a node or a node set  /   normally save large_feature node
        # state veactor
        self.user_embed = None
        self.conver_his = []  # conversation_history
        self.attr_ent = []  # attribute entropy

        self.ui_dict = self.__load_rl_data__(
            data_name, mode=mode)  # np.array [ u i weight]
        ...
        if mode == 'train':
            ...
            )  # init self.user_weight_dict  and  self.user_items_dict
        elif mode == 'test':
        ...
        if embeds:
            ...
            self.ui_embeds = nn.Embedding(self.user_length + self.item_length,
                                          64).weight.data.numpy()
            self.feature_emb = nn.Embedding(self.small_feature_length,
                                            64).weight.data.numpy()
    ...
    def __load_rl_data__(self, data_name, mode):
        if mode == 'train':
            with open(os.path.join(
                    DATA_DIR[data_name],
                    'UI_Interaction_data/review_dict_valid.json'),
                      encoding='utf-8') as f:
            ...
            with open(os.path.join(
                    DATA_DIR[data_name],
                    'UI_Interaction_data/review_dict_test.json'),
                      encoding='utf-8') as f:
                mydict = json.load(f)
        return mydict

    def __user_dict_init__(
            self
        ...
        for user_str in self.ui_dict.keys():
            ...
            self.user_weight_dict[user_id] = len(
                self.ui_dict[user_str]) / ui_nums
    ...
    def reset(self, embed=None):
        ...
        if self.mode == 'train':
            ...
            # self.user_id = np.random.choice(users, p=list(self.user_weight_dict.values()))
            # select user  according to user weights
            self.user_id = np.random.choice(users)
            self.target_item = np.random.choice(self.ui_dict[str(
                self.user_id)])
        elif self.mode == 'test':
        ...
        self.acc_small_fea = []
        self.rej_small_fea = []
        f_item = set(self.kg.G['item'][self.target_item]['belong_to_large'])
        i_belong = [
            item for item in range(self.item_length)
            if len(set(self.kg.G['item'][item]['belong_to_large']) -
                   f_item) == 0 and item != self.target_item
        ]

        self.cand_items = set(
            set(list(range(self.item_length))) - set(i_belong))
        # self.cand_items = list(range(self.item_length))
        print('user_id:{}, target_item:{}'.format(self.user_id,
                                                  self.target_item))

        # init  state vector
        self.user_embed = self.ui_embeds[
            self.user_id].tolist()  # init user_embed   np.array---list
        self.conver_his = [0] * self.max_turn  # conversation_history
        self.cand_len = [
            self.feature_length >> d & 1 for d in range(self.cand_item_num)
        ][::-1]
        self.attr_ent = [0] * self.attr_state_num  # attribute entropy

        # initialize dialog by randomly asked a question from ui interaction
        user_like_random_fea = random.choice(
            self.kg.G['item'][self.target_item]['belong_to_large'])
        self.user_acc_feature.append(
            user_like_random_fea)  # update user acc_fea
        ...
        print('=== init user prefer large_feature: {}'.format(
            self.cur_node_set))
        # self._update_cand_items(acc_feature=self.cur_node_set, rej_feature=[])
        self._update_feature_entropy()  # update entropy
        print('reset_reachable_feature num: {}'.format(
            len(self.reachable_feature)))
        ...
        cand_feature, cand_item, ac_label = self._get_cand()
        return self._get_scpr_state(), self._get_state(
        ), cand_feature, cand_item, ac_label, self._get_action_space()

    def _get_cand(self):
        if self.random_sample_feature:
            cand_feature = self._map_to_all_id(
                random.sample(self.reachable_feature,
                              min(len(self.reachable_feature), self.cand_num)),
                'feature')
        else:
            cand_feature = self._map_to_all_id(
                self._cand_small_feature(
                    self.reachable_feature[:self.cand_num]), 'feature')
        if self.random_sample_item:
            cand_item = self._map_to_all_id(
                random.sample(self.cand_items,
                              min(len(self.cand_items), self.cand_item_num)),
                'item')
        else:
            item_fld = []
            feature_fld = []
            item_o = []
            if len(self.cand_items) > 10:
                for item in self.cand_items:
                    # feature_groundtrue = self.kg.G['item'][self.target_item]['belong_to_large']
                    if len(self.kg.G['item'][item]['belong_to_large']) == len(
                            self.user_acc_feature) + 1:
                        fea = list(
                            set(self.kg.G['item'][item]['belong_to_large']) -
                            set(self.user_acc_feature))[0]
                        if fea not in feature_fld:
                            feature_fld.append(fea)
                            item_fld.append(item)
                        else:
                            item_o.append(item)
                    else:
                        item_o.append(item)
                # item_fld=[item for item in self.cand_items if
                # len(self.kg.G['item'][item]['belong_to'])==len(self.user_acc_feature)+1]
                cand_item = self._map_to_all_id(item_fld[:self.cand_item_num],
                                                'item')
                self.rej_fea = feature_fld[:self.cand_item_num]
            else:
                cand_item = self._map_to_all_id(
                    self.cand_items[:self.cand_item_num], 'item')
            if len(cand_item) < self.cand_item_num and len(cand_item) > 0:
                cand_item += self._map_to_all_id(
                    item_o[:self.cand_item_num - len(cand_item)], 'item')

            # cand_item = self._map_to_all_id(self.cand_items[:self.cand_item_num],'item')
        # cand = cand_feature + cand_item
        return cand_feature, cand_item, (self.target_item +
                                         self.user_length) in cand_item

    def _get_action_space(self):
        action_space = [
            self._map_to_all_id(self.reachable_small_feature, 'feature'),
            self._map_to_all_id(self.cand_items, 'item')
        ]
        return action_space

    def _get_scpr_state(self):
    ...
    def _get_state(self):
        user = [self.user_id]
        cur_node = [
            x + self.user_length + self.item_length for x in self.acc_small_fea
        ]
        cand_items = [x + self.user_length for x in self.cand_items]
        reachable_feature = [
            x + self.user_length + self.item_length
            for x in self.reachable_small_feature
        ]
        ...
        adj = torch.sparse.FloatTensor(
            i.t(), v, torch.Size([len(neighbors),
                                  len(neighbors)]))
    ...
    def step(self, action, sorted_actions, embed=None, is_sample=False):
        if embed is not None:
            self.ui_embeds = embed[:self.user_length + self.item_length]
            self.feature_emb = embed[self.user_length + self.item_length:]
        ...
        print('---------------step:{}-------------'.format(
            self.cur_conver_step))

        if self.cur_conver_step == self.max_turn:
            reward = self.reward_dict['until_T']
            self.conver_his[self.cur_conver_step -
                            1] = self.history_dict['until_T']
            ...
            asked_feature = self.kg.G['feature'][self._map_to_old_id(
                action)]['link_to_feature'][0]  # self.reachable_feature[0]
            print('-->action: ask features {}, max entropy feature {}'.format(
                asked_feature, self.reachable_feature[:self.cand_num]))
            reward, done, acc_rej = self._ask_update(
                asked_feature
            )  # update user's profile:  user_acc_feature & user_rej_feature
            self._update_cand_items(asked_feature,
                                    acc_rej)  # update cand_items
            ...
            if not done and not is_sample:
                self._update_cand_items(self.rej_fea, False)
                self.user_rej_feature = self.user_rej_feature + self.rej_fea
            # ========================================
            if reward > 0:
        ...
        )  # update user's profile: reachable_feature
        ...
        if len(self.reachable_feature
               ) != 0:  # if reachable_feature == 0 :cand_item= 1
    ...
    def _updata_reachable_feature(self, start='large_feature'):
        ...
        for fea in self.user_acc_feature + self.user_rej_feature:
            filter_small_feature.extend(
                self.kg.G['large_feature'][fea]['link_to_feature'])
        filter_small_feature = set(filter_small_feature)

        for cand in self.cand_items:
            fea_belong_items = list(
                self.kg.G['item'][cand]['belong_to_large'])  # A-I
            ...
            reachable_item_small_feature_pair[cand] = list(
                set(small_fea_belong_items) - filter_small_feature -
                self.invalid_small_feature) + self.acc_small_fea
            next_reachable_feature = list(set(next_reachable_feature))
            next_reachable_small_feature = list(
                set(next_reachable_small_feature))
        self.reachable_feature = list(
            set(next_reachable_feature) - set(self.user_acc_feature) -
            set(self.user_rej_feature))
        self.reachable_small_feature = list(
            set(next_reachable_small_feature) - filter_small_feature -
            self.invalid_small_feature)
    ...
    def _item_score(self):
        ...
        for item_id in self.cand_items:
            ...
            prefer_embed = self.feature_emb[
                self.acc_small_fea, :]  # np.array (x*64), small_feature
            unprefer_feature = list(
                set(self.rej_small_fea)
                & set(self.kg.G['item'][item_id]['belong_to']))
            unprefer_embed = self.feature_emb[
                unprefer_feature, :]  # np.array (x*64)
            for i in range(len(self.acc_small_fea)):
                score += np.inner(prefer_embed[i], item_embed)
            for i in range(len(unprefer_feature)):
                score -= self.sigmoid(
                    [np.inner(unprefer_embed[i], item_embed)])[0]
    ...
    def _ask_update(self, asked_feature):
        ...
        feature_groundtrue = self.kg.G['item'][
            self.target_item]['belong_to_large']
        ...
        if asked_feature in feature_groundtrue:
            ...
            self.conver_his[self.cur_conver_step] = self.history_dict[
                'ask_suc']  # update conver_his
        else:
            acc_rej = False
            self.user_rej_feature.append(asked_feature)
            reward = self.reward_dict['ask_fail']
            self.conver_his[self.cur_conver_step] = self.history_dict[
                'ask_fail']  # update conver_his
    ...
    def _update_cand_items(self, asked_feature, acc_rej):
        small_feature_groundtrue = self.kg.G['item'][self.target_item][
            'belong_to']  # TODO small_ground truth
        ...
        if acc_rej:  # accept feature
            ...
            feature_small_ids = self.kg.G['large_feature'][asked_feature][
                'link_to_feature']
            ...
            for small_id in feature_small_ids:
                if small_id in small_feature_groundtrue:  # user_accept small_tag
                    ...
                    self.cand_items = set(self.cand_items) & set(
                        feature_items)  # itersection
            ...
            if isinstance(asked_feature, list):
                feature_items = []
                for fea in asked_feature:
                    feature_small_ids = self.kg.G['large_feature'][fea][
                        'link_to_feature']
                    for small_id in feature_small_ids:
                        self.rej_small_fea.append(small_id)
                        feature_items += self.kg.G['feature'][small_id][
                            'belong_to']
            else:
                feature_items = []
                feature_small_ids = self.kg.G['large_feature'][asked_feature][
        ...
        self.cand_len = [
            len(self.cand_items) >> d & 1 for d in range(self.cand_item_num)
        ][::-1]
    ...
    def _recommend_update(self, recom_items):
        ...
        if self.target_item in recom_items:
            ...
            self.conver_his[self.cur_conver_step] = self.history_dict[
                'rec_scu']  # update state vector: conver_his
            tmp_score = []
            ...
            self.conver_his[self.cur_conver_step] = self.history_dict[
                'rec_fail']  # update state vector: conver_his
            if len(self.cand_items) > self.rec_num:
                ...
                self.cand_len = [
                    len(self.cand_items) >> d & 1
                    for d in range(self.cand_item_num)
                ][::-1]
            done = 0
        return reward, done
    ...
    def _update_feature_entropy(self):
        if self.ent_way == 'entropy':
            ...
            # TODO Dataframe
            for item_id in self.cand_items:
                cand_items_fea_list.append(
                    list(self.kg.G['item'][item_id]['belong_to']))
            ...
            for large_fea_id in real_ask_able_large_fea:
                large_ent = 0
                small_feature = list(self.kg.G['large_feature'][large_fea_id]
                                     ['link_to_feature'])
                small_feature_in_cand = list(
                    set(small_feature) & set(self.attr_count_dict.keys()))

                for fea_id in small_feature_in_cand:
                    p1 = float(self.attr_count_dict[fea_id]) / len(
                        self.cand_items)
                    p2 = 1.0 - p1
                    if p1 == 1:
                        large_ent += 0
            ...
            # cand_item_score = self._item_score()
            cand_item_score_sig = self.sigmoid(
                self.cand_item_score)  # sigmoid(score)

            for score_ind, item_id in enumerate(self.cand_items):
                cand_items_fea_list = list(
                    self.kg.G['item'][item_id]['belong_to'])
                for fea_id in cand_items_fea_list:
                    if self.attr_count_dict.get(fea_id) is None:
                        self.attr_count_dict[fea_id] = 0
                    self.attr_count_dict[fea_id] += cand_item_score_sig[
                        score_ind]
    ...
    def _cand_small_feature(self, cand_features):
        ...
        for fea in cand_features:
            cand_small_features.extend(
                self.kg.G['large_feature'][fea]['link_to_feature'])
        # print(list(set(cand_small_features) & set(self.reachable_small_feature)))
        return list(
            set(cand_small_features) & set(self.reachable_small_feature))
