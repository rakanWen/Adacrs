...
import random
import torch
from adacrs.utils.utils_load_save import set_random_seed, load_embed
from adacrs.utils.utils_load_save import DATA_DIR
...
class BinaryRecommendEnv(object):
    ...
    def __init__(self,
        ...
        if embeds:
            ...
            self.feature_emb = nn.Embedding(self.feature_length,
                                            64).weight.data.numpy()
        # self.feature_length = self.feature_emb.shape[0]-1

        self.action_space = 2
        self.state_space = self.cand_item_num + self.max_turn
        self.reward_dict = {
            'ask_suc': 0.01,
            'ask_fail': -0.1,
            'rec_suc': 1,
    ...
    def reset(self, embed=None):
        ...
        self.user_acc_feature = []  # user accepted feature which asked by agent
        self.user_rej_feature = []  # user rejected feature which asked by agent
        f_item = set(self.kg.G['item'][self.target_item]['belong_to'])
        i_belong = [
            item for item in range(self.item_length)
            if len(set(self.kg.G['item'][item]['belong_to']) -
        ...
        user_like_random_fea = random.choice(
            self.kg.G['item'][self.target_item]['belong_to'])
    ...
    def _get_cand(self):
        ...
        if self.random_sample_item:
            ...
            feature_fld = []
            item_o = []
            if len(self.cand_items) > 20:
                for item in self.cand_items:
                    if len(self.kg.G['item'][item]['belong_to']) == len(
                            self.user_acc_feature) + 1:
                        fea = list(
                            set(self.kg.G['item'][item]['belong_to']) -
                ...
                if len(cand_item) > 0:
                    ac_label = True
                if len(cand_item) < self.cand_item_num:
                    cand_item += self._map_to_all_id(
        ...
        # cand = cand_feature + cand_item
            if (self.target_item + self.user_length) in cand_item:
                ac_label = True
        return cand_feature, cand_item, ac_label

    def _get_action_space(self):
        action_space = [
            self._map_to_all_id(self.reachable_feature, 'feature'),
    ...
    def _get_state(self):
        if self.data_name in ['YELP_STAR', 'INS']:
        ...
        user = [self.user_id]
        cur_node = [
            x + self.user_length + self.item_length for x in self.cur_node_set
        ]
        cand_items = [x + self.user_length for x in self_cand_items]
        reachable_feature = [
            x + self.user_length + self.item_length
            for x in self.reachable_feature
        ]
    ...
    def step(self, action: int, sorted_actions: List[int], embed=None, is_sample: bool = False):
        if embed is not None:
            self.ui_embeds = embed[:self.user_length + self.item_length]
            self.feature_emb = embed[self.user_length + self.item_length:]
        ...
        """ if self.cur_conver_step <= 5:
            #asked_feature = self._map_to_old_id(self.reachable_feature[0])
            # self._map_to_old_id(action)
            asked_feature = self._map_to_old_id(action)
            print('-->action: hard ask features {}, max entropy feature {}'.format(asked_feature,
    ...
    def _updata_reachable_feature(self):
        ...
        for cand in self.cand_items:
            fea_belong_items = list(
                self.kg.G['item'][cand]['belong_to'])  # A-I
            next_reachable_feature.extend(fea_belong_items)
            reachable_item_feature_pair[cand] = list(
                set(fea_belong_items) - set(self.user_rej_feature))
    ...
    def _item_score(self):
        ...
        for item_id in self.cand_items:
            ...
            prefer_embed = self.feature_emb[
                self.user_acc_feature, :]  # np.array (x*64)
            unprefer_feature = list(
                set(self.user_rej_feature) & set(self.kg.G['item'][item_id]['belong_to']))
    ...
    def _ask_update(self, asked_feature: int):
        '''
    ...
    def _update_cand_items(self, asked_feature: int, acc_rej: bool):
        if acc_rej:  # accept feature
            ...
            if isinstance(asked_feature, list):
                ...
                feature_items = self.kg.G['feature'][asked_feature][
                    'belong_to']
            self.cand_items = set(self.cand_items) - set(
                feature_items)  # itersection
            self.cand_items = list(self.cand_items)
    ...
    def _recommend_update(self, recom_items: List[int]):
    ...
    def _update_feature_entropy(self):
        if self.ent_way == 'entropy':
            ...
            real_ask_able = list(
                set(self.reachable_feature) & set(self.attr_count_dict.keys()))
    ...
    def sigmoid(self, x_list: Tuple[np.float64]):
        x_np = np.array(x_list)
        s = 1 / (1 + np.exp(-x_np))
        return s.tolist()

    def _map_to_all_id(self, x_list: List[int], old_type: str):
        if old_type == 'item':
