...
class LastFmStarDataset(Processor):
    ...
    def load_entities(self):
        entity_files = edict(
            user='user_dict.json',
            item='item_dict.json',
            feature='original_tag_map.json', # differ from lastfm
        )
    ...
    def load_relations(self):
        ...
        for name in LastFm_relations:
            ...
            relation = edict(data=[], )
            knowledge = [
                list([]) for i in range(LastFm_relations[name][1].value_len)
            ]
            # load relation files
            with open(os.path.join(self.data_dir, LastFm_relations[name][0]),
                      encoding='utf-8') as f:
                mydict = json.load(f)
            if name in ['interact']:
