...
class YelpGraph(Graph):
    ...
    def _load_entities(self, dataset):
        print('load entities...')
        num_nodes = 0
        data_relations, _, _ = dataset.get_relation(
        )  # entity_relations, relation_name, link_entity_type
        entity_list = list(data_relations.keys()) # user, item, feature, large_feature
        for entity in entity_list:
            ...
            print('load entity:{:s}  : Total {:d} nodes.'.format(
                entity, entity_size))
        ...
        for relation in data_relations_name:
            ...
            for he_id, te_ids in enumerate(data):  # head_entity_id , tail_entity_ids
                ...
                for te_id in set(te_ids):
                    self._add_edge(e_head_type, he_id, relation, e_tail_type,
                                   te_id)
                    num_edges += 2
