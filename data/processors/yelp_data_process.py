...
class YelpDataset(Processor):
    ...
    def get_relation(self):
        ...
        LIKE = 'like'
        BELONG_TO = 'belong_to'
        BELONG_TO_LARGE = 'belong_to_large'  # feature(second-layer tag) -->
        # large_feature(first-layer tag)
        LINK_TO_FEATURE = 'link_to_feature'  # large_feature(first-layer tag)  -->
        # feature(second-layer tag)
        relation_name = [
            INTERACT, FRIEND, LIKE, BELONG_TO, BELONG_TO_LARGE, LINK_TO_FEATURE
        ]
    ...
    def load_entities(self):
        ...
        for entity_name in entity_files:
            with open(os.path.join(self.data_dir, entity_files[entity_name]),
                      encoding='utf-8') as f:
                mydict = json.load(f)
            ...
            setattr(self, entity_name,
                    edict(id=entity_id, value_len=max(entity_id) + 1))
    ...
    def load_relations(self):
        ...
        Yelp_relations = edict(
            interact=('user_item.json', self.user,
                      self.item),  # (filename, head_entity, tail_entity)
            ...
            belong_to_large=('item_dict-merged_tag.json', self.item,
                             self.large_feature),
            link_to_feature=('2-layer taxonomy.json', self.large_feature,
                             self.feature))
        for name in Yelp_relations:
            #  Save tail_entity
            relation = edict(data=[], )
            knowledge = [
                list([]) for i in range(Yelp_relations[name][1].value_len)
            ]
            # load relation files
            with open(os.path.join(self.data_dir, Yelp_relations[name][0]),
                      encoding='utf-8') as f:
                mydict = json.load(f)
